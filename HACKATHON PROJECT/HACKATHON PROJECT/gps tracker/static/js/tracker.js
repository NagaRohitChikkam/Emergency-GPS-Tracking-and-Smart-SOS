...
function showLocationPermissionModal() {
  ...
  const existing = document.getElementById('gps-permission-modal');
  ...
  if (isChrome || isEdge) {
    browserSteps = `
      <li>Click the <strong>🔒 lock icon</strong> (or ⓘ) in the address bar</li>
      <li>Click <strong>"Site settings"</strong></li>
      <li>Find <strong>"Location"</strong> and set it to <strong>"Allow"</strong></li>
      <li>Reload this page and click <strong>Start Tracking</strong> again</li>`;
  } else if (isFirefox) {
    browserSteps = `
      <li>Click the <strong>🔒 lock icon</strong> in the address bar</li>
      <li>Click the <strong>arrow ›</strong> next to "Connection Secure"</li>
      <li>Click <strong>"More Information"</strong> → <strong>"Permissions"</strong></li>
      <li>Set <strong>"Access Your Location"</strong> to <strong>"Allow"</strong></li>
      <li>Reload this page and try again</li>`;
  } else if (isSafari) {
    browserSteps = `
      <li>Open <strong>System Settings → Privacy & Security → Location Services</strong></li>
      <li>Enable Location Services for <strong>Safari</strong></li>
      <li>Return to this page and reload</li>`;
  } else {
    browserSteps = `
      <li>Go to your browser <strong>Settings → Site Permissions → Location</strong></li>
      <li>Find this site and set it to <strong>Allow</strong></li>
      <li>Reload this page and try again</li>`;
  }

  const httpsNote = (!isHTTPS && !isLocalhost) ? `
    <div style="background:rgba(245,158,11,0.15);border:1px solid rgba(245,158,11,0.4);border-radius:10px;padding:14px;margin-top:14px;">
      <strong style="color:#f59e0b">⚠️ HTTP Connection Detected</strong><br>
      <span style="font-size:.82rem;color:var(--text-secondary)">
        Browsers <strong>block GPS on non-HTTPS pages</strong> for security reasons.<br>
        You are using: <code style="background:rgba(255,255,255,0.08);padding:2px 6px;border-radius:4px">${location.origin}</code><br>
        For GPS to work on mobile/LAN, access via <strong>localhost:5000</strong> on this PC, or enable HTTPS.
      </span>
    </div>` : '';
  ...
  modal.innerHTML = `
    ...
    " onclick="if(event.target===this)this.remove()">
      <div style="
        background:var(--surface,#12121a);border:1px solid rgba(239,68,68,0.4);
      ...
      ">
        ...
        <h2 style="margin:0 0 8px;text-align:center;font-size:1.25rem;color:#ef4444">
          Location Access Blocked
        </h2>
        <p style="color:var(--text-secondary,#aaa);font-size:.875rem;text-align:center;margin-bottom:20px">
          GPS tracking requires location permission. Here's how to enable it:
        </p>
        <div style="background:rgba(255,255,255,0.04);border-radius:12px;padding:16px;margin-bottom:16px">
          <ol style="margin:0;padding-left:20px;color:var(--text-primary,#e0e0e0);font-size:.875rem;line-height:1.9">
            ${browserSteps}
          </ol>
        </div>
        ${httpsNote}
        <div style="margin-top:20px;display:flex;gap:10px;flex-wrap:wrap">
          <button id="use-ip-btn" style="
            flex:1;padding:12px;border-radius:10px;border:1px solid rgba(255,255,255,0.15);
            background:rgba(255,255,255,0.06);color:#e0e0e0;cursor:pointer;font-size:.85rem;
          ">📡 Use IP Location Instead</button>
          <button onclick="location.reload()" style="
            flex:1;padding:12px;border-radius:10px;border:none;
            background:linear-gradient(135deg,#6366f1,#8b5cf6);
            color:#fff;cursor:pointer;font-weight:600;font-size:.875rem;
          ">🔄 Reload Page</button>
        </div>
        <button onclick="document.getElementById('gps-permission-modal').remove()" style="
          display:block;width:100%;margin-top:10px;padding:10px;
          background:transparent;border:none;color:var(--text-muted,#666);
          cursor:pointer;font-size:.8rem;
        ">✕ Dismiss — use 📍 Set Manually button on the map</button>
      </div>
    </div>
  `;
  document.body.appendChild(modal);

  document.getElementById('use-ip-btn').addEventListener('click', () => {
    document.getElementById('gps-permission-modal').remove();
    fetchIPLocation().then(() =>
      showToast('📡 IP Location Active', 'Using approximate IP location. Click 📍 Set Manually on the map for better accuracy.', 'info', 8000)
    );
  });

  // Auto-load IP location as fallback
  fetchIPLocation();
}
...
function startTracking() {
  ...
  state.watchId = navigator.geolocation.watchPosition(async (pos) => {
    ...
    if (err.code === 1) {
      // Permission denied — show detailed fix instructions
      showLocationPermissionModal();
      // We keep state.isTracking = true internally so manual SOS still works with IP location,
      // but update the button to "Stop Tracking" state.
      updateTrackingBtn(true);
    } else {
      const msgs = {
...
// ── SOS Button ───────────────────────────────────────────────────────────────
window.triggerSOS = async function (skipConfirm = false) {
  if (!skipConfirm) {
